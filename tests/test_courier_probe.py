...
@pytest.mark.asyncio
async def test_courier_probe_write_ack():
    from katzenpost_status.courier_probe import probe_provider
...
@pytest.mark.asyncio
async def test_courier_probe_all_providers():
    from katzenpost_status.courier_probe import probe_all_providers

    client = await setup_thin_client()
    config_path = get_config_path()
    try:
        results = await probe_all_providers(
            config_path, client, timeout=60.0, debug=True
        )

        assert len(results) > 0, "No courier providers found"
