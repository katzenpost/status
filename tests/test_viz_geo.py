...
def test_curated_override_by_ip_and_name():
    r = viz_geo.GeoResolver(overrides={
        "1.2.3.4": [10.0, 20.0, "City A"],
        "annares": [1.0, 2.0, "Named"],
    })
    assert r.resolve("1.2.3.4") == {"lat": 10.0, "lon": 20.0, "label": "City A", "source": "curated"}
...
def test_shipped_overrides_load_and_are_well_formed():
    ...
    for key, val in ov.items():
        ...
        assert isinstance(val[0], (int, float)) and isinstance(val[1], (int, float))
...
def test_missing_database_degrades_without_crashing():
    ...
    r = viz_geo.GeoResolver(db_path="/no/such/City.mmdb", asn_db_path="/no/such/ASN.mmdb")
    assert r.resolve_asn("8.8.8.8") is None
    # a curated override still resolves without any database
    r2 = viz_geo.GeoResolver(db_path="/no/such/City.mmdb", overrides={"nodeX": [1.0, 2.0, "Town"]})
...
def test_asn_whois_uses_cache_without_network(tmp_path):
    ...
    cache.write_text(json.dumps({
        "1.2.3.4": {"asn": "AS64500", "org": "Test Net"},   # positive, cached
