...
_SNAPSHOT = json.loads(
    (Path(__file__).resolve().parents[1] / "testdata" / "consensus_namenlos.json").read_text()
)
DOC = _SNAPSHOT["doc"]
...
def _tr(reachable, latency, nhops):
    return {
        "reachable": reachable, "final_latency_ms": latency,
        "hops": [{"hop": i + 1, "ip": f"10.0.{i}.{i}", "latency_ms": round((i + 1) * latency / nhops, 1)}
                 for i in range(nhops)] if reachable else [],
    }


def _survey_entry(name, node_type, reachable=True, latency=15.0, nhops=6, host="198.51.100.1", port=1):
    return {f"{name}|{node_type}|{host}:{port}": {
        "name": name, "node_type": node_type, "host": host, "port": port,
        "tcp_traceroute": _tr(reachable, latency, nhops),
        "icmp_ping": {"reachable": reachable, "latency_ms": latency - 2 if reachable else None},
    }}
...
def _full_survey():
    ...
    for i, layer in enumerate(DOC["Topology"]):
        for node in layer:
            survey.update(_survey_entry(node["Name"], f"mix-L{i}", nhops=8 + i))
    return survey


def _payload(survey_results=None, dirauth_status=None):
    return viz.build_viz_payload(
        DOC, survey_results=survey_results,
        dirauth_status=dirauth_status if dirauth_status is not None else DIRAUTH_STATUS,
        network_name="namenlos", epoch=DOC["Epoch"],
        epoch_time_str=_SNAPSHOT["captured_epoch_time"], generated_at="2026-09-01T00:00:00Z",
    )
...
def test_vantage_and_client_count_in_payload():
    p = viz.build_viz_payload(
        DOC, dirauth_status=DIRAUTH_STATUS, network_name="namenlos",
        vantage={"lat": 52.0, "lon": 5.0, "label": "monitor"}, clients_per_gateway=4,
    )
...
def test_node_geo_from_curated_resolver():
    ...
    r = viz_geo.GeoResolver(overrides={"192.87.90.45": [51.44, 5.48, "Eindhoven"]})
    idx = _index(viz.build_viz_payload(
        DOC, dirauth_status=DIRAUTH_STATUS, network_name="namenlos", geo_resolver=r)["nodes"])
...
def test_in_consensus_node_is_ok_and_carries_path():
    survey = _survey_entry("windfallgw", "gateway", reachable=True, latency=12.3, nhops=25)
    gw = _index(_payload(survey_results=survey)["nodes"])[("windfallgw", "gateway")]
    assert gw["status"] == "ok" and gw["reachable"] and gw["hop_count"] == 25 and gw["latency_ms"] == 12.3


def test_dropped_reachable_node_is_out_with_address():
    survey = _survey_entry("mixy", "mix-L1", reachable=True, latency=30.0, nhops=9, host="203.0.113.9")
    doc = json.loads(json.dumps(DOC))
    doc["Topology"][1] = [n for n in doc["Topology"][1] if n["Name"] != "mixy"]
    idx = _index(viz.build_viz_payload(doc, survey_results=survey, dirauth_status=DIRAUTH_STATUS)["nodes"])
    assert ("mixy", "out") in idx
    out = idx[("mixy", "out")]
    assert out["status"] == "out" and out["details"]["addresses"] == ["tcp://203.0.113.9:1"]
...
def _generate(tmp_path, **kw):
    ...
    viz.generate_viz(DOC, str(out), survey_results=_full_survey(), dirauth_status=DIRAUTH_STATUS,
                     network_name="namenlos", epoch=DOC["Epoch"],
                     epoch_time_str=_SNAPSHOT["captured_epoch_time"], **kw)
    return out
...
def test_only_data_file_changes_across_runs(tmp_path):
    ...
    viz.generate_viz(DOC, str(tmp_path / "visualize.html"), survey_results=_full_survey(),
                     dirauth_status=DIRAUTH_STATUS, network_name="namenlos",
                     epoch=DOC["Epoch"], epoch_time_str=_SNAPSHOT["captured_epoch_time"])
...
class _StubResolver:
    ...
    def resolve(self, ip, name=None):
        return {"lat": 1.0, "lon": 2.0, "label": "Testville, Testland", "source": "stub"} if ip else None
...
def test_epoch_timing_fields_in_payload():
    p = viz.build_viz_payload(
        DOC, survey_results=_full_survey(), dirauth_status=DIRAUTH_STATUS,
        epoch=DOC["Epoch"], epoch_end="2026-09-02T00:20:00Z", epoch_period_s=1200.0,
    )
...
def test_node_asn_present_when_resolver_has_asn():
    ...
    assert with_asn, "at least one node should carry an AS when the resolver has one"
    assert with_asn[0]["asn"] == "AS64500"
    assert with_asn[0]["net"] == "Test Net"


def _hpayload(epoch, gen):
    return {"epoch": epoch, "generated_at": gen, "epoch_time_str": "t",
            "counts": {"total": 1}, "nodes": [{"name": "a", "type": "mix", "status": "ok"}]}


def test_history_writer_dedupes_and_prunes(tmp_path):
    for e in (1, 2, 3):
        viz._write_history(tmp_path, "visualize", _hpayload(e, f"g{e}"),
                           json.dumps(_hpayload(e, f"g{e}")), history_len=2)
    hist = tmp_path / "visualize-history"
    index = json.loads((hist / "index.json").read_text())
    assert [x["epoch"] for x in index] == [2, 3]          # pruned to the last 2 epochs
    ...
    viz._write_history(tmp_path, "visualize", _hpayload(3, "g3b"),
                       json.dumps(_hpayload(3, "g3b")), history_len=2)
    index = json.loads((hist / "index.json").read_text())
