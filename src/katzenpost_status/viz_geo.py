...
class GeoResolver:
    ...
    def __init__(
        ...
        self._asn_cache_path = Path(asn_cache_path) if asn_cache_path else None
        self._asn_cache: dict[str, Any] = {}
        if self._asn_cache_path and self._asn_cache_path.exists():
            try:
                loaded = json.loads(self._asn_cache_path.read_text(encoding="utf-8"))
    ...
    def _asn_via_whois(self, ip: str) -> dict[str, Any] | None:
        ...
        if self._asn_cache_path:
            try:
                ...
                self._asn_cache_path.write_text(json.dumps(self._asn_cache), encoding="utf-8")
            except OSError:
                pass
        return result
    ...
    def _from_override(self, key: str | None) -> dict[str, Any] | None:
        ...
        return {"lat": float(v[0]), "lon": float(v[1]), "label": label, "source": "curated"}
    ...
    def _from_db(self, ip: str | None) -> dict[str, Any] | None:
        ...
        country: Any = ((rec.get("country") or {}).get("names") or {}).get("en", "")
        label = ", ".join(str(x) for x in (city, country) if x)
        return {"lat": float(lat), "lon": float(lon), "label": label, "source": "geoip-db"}

    def resolve(self, ip: str | None, name: str | None = None) -> dict[str, Any] | None:
