(function () {
    ...
    anchorsEdges('starflower', 'Decagonal rosette (Girih-like)', function (a, e, T, color) { var i, j; function rosette(cx, cy, R) { var n = 10, outer = [], k; for (k = 0; k < n; k++) { var th = k / n * PI2, p = new T.Vector3(cx + Math.cos(th) * R, cy + Math.sin(th) * R, 0); outer.push(p); a.push(p); } for (k = 0; k < n; k++) { e.push({ a: outer[k], b: outer[(k + 1) % n], color: color }); e.push({ a: outer[k], b: outer[(k + 3) % n], color: color }); e.push({ a: outer[k], b: outer[(k + 4) % n], color: color }); } } for (i = -1; i <= 1; i++) for (j = -1; j <= 1; j++) rosette(i * 15, j * 15 + (i & 1 ? 7.5 : 0), 7); }, 0x9b5de5, 58);

    anchorsEdges('moiregrid', 'Moire interference grid', function (a, e, T, color) { var R = 22, lines = 26, k; function grid(rot) { var ca = Math.cos(rot), sa = Math.sin(rot); for (var i = 0; i < lines; i++) { var u = -R + 2 * R * i / (lines - 1); var x1 = u * ca - (-R) * sa, y1 = u * sa + (-R) * ca, x2 = u * ca - R * sa, y2 = u * sa + R * ca; var p1 = new T.Vector3(x1, y1, 0), p2 = new T.Vector3(x2, y2, 0); a.push(p1); a.push(p2); e.push({ a: p1, b: p2, color: color }); } } grid(0); grid(0.14); }, 0x4d8bf0, 56);
})();
