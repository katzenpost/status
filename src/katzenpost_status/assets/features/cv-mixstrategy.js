...
mk('strat-cascade','Cascade vs free-route: cascade',0x2ec4b6,62,undefined,function(d,T){return pipe(d,T,{edge:'matched',color:0x2ec4b6});});mk('strat-freeroute','Free-route full bipartite',0x4d8bf0,62,undefined,function(d,T){return pipe(d,T,{});});mk('strat-stratified','Stratified topology',0x9b5de5,62,undefined,function(d,T){return pipe(d,T,{place:function(ci,j,n,fr){return [(fr-0.5)*22,(ci%2?4:-4)];}});});mk('strat-restricted','Restricted-routes sparse graph',0xff5d8f,62,undefined,function(d,T){var lay=pipe(d,T,{}),edges=lay.edges.filter(function(e,i){return (i*7)%3===0;});return {nodes:lay.nodes,edges:edges};});mk('strat-butterfly','Butterfly network',0xff8f3f,62,undefined,function(d,T){return pipe(d,T,{edge:'matched',place:function(ci,j,n,fr){var a=(fr-0.5)*Math.PI;return [12*Math.sin(a+ci*0.5),8*Math.cos(a)];},after:function(nodes,edges,cp){var e,i,j;for(e=0;e<cp.length-1;e++){var A=cp[e],B=cp[e+1];for(i=0;i<A.length;i++)for(j=0;j<B.length;j++)if(((i^j)&1)===0)edges.push({a:A[i],b:B[j],color:pc(e+3)});}}});});mk('strat-expander','Expander graph',0xd2a0,62,undefined,function(d,T){return pipe(d,T,{place:function(ci,j,n){var a=PI2*j/n;return [11*Math.sin(a),11*Math.cos(a)];},after:function(nodes,edges){var i;for(i=0;i<nodes.length;i++){var b=nodes[(i*2+1)%nodes.length];edges.push({a:nodes[i].pos,b:b.pos,color:pc(i)});}}});});mk('strat-bipartite','Complete-bipartite layers',0xffd23f,62,undefined,function(d,T){return pipe(d,T,{color:0xffd23f});});mk('strat-uniform','Uniform path-selection fan',0x33ccff,62,undefined,function(d,T){return pipe(d,T,{place:function(ci,j,n,fr){var a=(fr-0.5)*Math.PI*0.9;return [13*Math.sin(a),0];}});});mk('strat-widthdepth','Width vs depth',0xff5d6c,62,undefined,function(d,T){return pipe(d,T,{X:16,place:function(ci,j,n,fr){return [(fr-0.5)*Math.max(6,n*5),0];}});});mk('strat-bottleneck','Narrowest-layer bottleneck',0x8a5bff,62,undefined,function(d,T){var cols=G.columns(d),minw=1e9,mi=0,ci;for(ci=0;ci<cols.length;ci++)if(cols[ci].length<minw){minw=cols[ci].length;mi=ci;}return pipe(d,T,{place:function(c,j,n,fr){var pinch=(c===mi)?0.35:1;return [(fr-0.5)*22*pinch,0];},color:0x8a5bff,after:function(nodes,edges,cp){var A=cp[mi]||[];A.forEach(function(p){edges.push({a:V3(T,p.x,p.y,-4),b:V3(T,p.x,p.y,4),color:0xff5d6c});});}});});
})();
