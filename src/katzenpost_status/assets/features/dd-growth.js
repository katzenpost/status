(function () {
    ...
    profileView('gr-ballistic', 'Ballistic deposition interface', function () { var W = 200, h = new Float32Array(W), i, drops = W * 34; for (i = 0; i < drops; i++) { var x = (Math.random() * W) | 0, m = h[x] + 1; if (x > 0) m = Math.max(m, h[x - 1]); if (x < W - 1) m = Math.max(m, h[x + 1]); h[x] = m; } return h; }, 0x2ec4b6, 58);

    profileView('gr-rsos', 'KPZ interface (RSOS)', function () { var W = 200, h = new Int32Array(W), i, drops = W * 60; for (i = 0; i < drops; i++) { var x = (Math.random() * W) | 0, ok = true; if (x > 0 && (h[x] + 1 - h[x - 1]) > 1) ok = false; if (x < W - 1 && (h[x] + 1 - h[x + 1]) > 1) ok = false; if (ok) h[x] += 1; } return h; }, 0x4d8bf0, 58);
})();
