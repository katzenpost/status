(function () {
    ...
    ae('eulertonnetz', 'Euler Tonnetz (3D)', function (a, e, T, color) { var R = 2, s = 7, i, j, k; function P(i, j, k) { return new T.Vector3(i * s + k * 2, j * s * 0.9, k * s * 0.7); } for (i = -R; i <= R; i++) for (j = -R; j <= R; j++) for (k = -1; k <= 1; k++) { a.push(P(i, j, k)); if (i < R) e.push({ a: P(i, j, k), b: P(i + 1, j, k), color: color }); if (j < R) e.push({ a: P(i, j, k), b: P(i, j + 1, k), color: 0x4d8bf0 }); if (k < 1) e.push({ a: P(i, j, k), b: P(i, j, k + 1), color: 0xff8f3f }); } }, 0x00d2a0, 58);
})();
