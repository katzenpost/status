...
Results are returned for each courier->replica pair. Since the courier chooses
which replica to use internally, we cannot test specific replicas. The same
result applies to all replicas for a given courier.

Possible results:
- SUCCESS: full round-trip worked
- ACK_ONLY: courier ACK received but no data from replica
- NO_REPLICAS: no storage replicas in PKI consensus, cannot test
...
async def probe_via_courier(
    ...
    if debug:
        logger.info(
            "[%s] replica probe: starting round-trip test",
            provider_name,
        )
    ...
    try:
        ...
        if debug:
            logger.error(
                "[%s] replica probe: daemon connection timeout",
                provider_name,
            )
        ...
        if debug:
            logger.error(
                "[%s] replica probe: daemon connection failed: %s",
                provider_name,
                e,
            )
        thinclient_logger.setLevel(original_level)
    ...
    try:
        ...
        if debug:
            logger.info(
                "[%s] replica probe: sending write to courier", provider_name
            )
        ...
        try:
            ...
            if debug:
                logger.info(
                    "[%s] replica probe: write ACK received", provider_name
                )
        ...
        if debug:
            logger.info(
                "[%s] replica probe: waiting %.0fs for replication",
                provider_name,
                replication_wait,
            )
        ...
        for attempt in range(max_read_attempts):
            if debug:
                logger.info(
                    "[%s] replica probe: read attempt %d/%d",
                    provider_name,
                    attempt + 1,
                    max_read_attempts,
                )
            ...
            try:
                ...
                if (
                    ...
                    and getattr(read_result, "plaintext", None) is not None
                ):
                    if debug:
                        logger.info(
                            "[%s] replica probe: got read response",
                            provider_name,
                        )
                ...
                if debug:
                    logger.info(
                        "[%s] replica probe: empty response, retrying",
                        provider_name,
                    )
                ...
                logger.warning(
                    "[%s] replica probe: read ERROR: %s: %s",
                    provider_name,
                    type(e).__name__,
                    e,
                )
        ...
        if got_read_response:
            if debug:
                logger.info(
                    "[%s] replica probe: SUCCESS - round-trip complete",
                    provider_name,
                )
...
async def probe_all_replicas(
    ...
    try:
        ...
        if debug:
            logger.error(
                "replica probe: failed to get courier services: %s", e
            )
        return {}
    ...
    for courier_name, result in zip(courier_tasks.keys(), gathered):
        if isinstance(result, BaseException):
            if debug:
                logger.error(
                    "[%s] replica probe: task exception: %s",
                    courier_name,
                    result,
                )
            ...
            if debug:
                ...
                logger.info(
                    "[%s] replica probe: result=%s latency=%s",
                    courier_name,
                    result_code,
                    latency_str,
                )
    ...
    if debug:
        success_count = sum(
            1
            for code, _ in courier_results.values()
